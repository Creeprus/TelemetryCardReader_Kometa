...
QStringList MainWindow::validateObject(QJsonObject obj)
{
    ...
    {
        validationError.append("Ошибки валидации на устройстве с id: " +obj.value("id времени").toString());
    }
    for (int j = 0; j < obj.size(); j++) {
        auto key = obj.keys().at(j);
        auto value = obj.value(key);
        ...
        if (key == "биты" ) {
            ...
            if (convertable == false) {
                ...
                {
                     validationError.append("Биты обладают неправильным разделителем. Должен быть -");  break;
                }
            ...
            // Тут проверит на уникальность биты в значениях
